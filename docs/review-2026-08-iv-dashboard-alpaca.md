...
### Suivi des correctifs (2026-08-21, même branche, commit suivant)

Les cinq majeurs ont été traités en TDD (test rouge observé avant chaque correctif, `tests/test_iv_dashboard_service.py`, marqueur `unit`) :

| # | Correctif appliqué | Test qui le verrouille |
|---|---|---|
| M1 | Bloc « fallback chaîne complète » supprimé de `fetch_current_atm_iv` (et `_contracts_from_chain_df` avec lui) : un échec des snapshots remonte la cause réelle, bornée à une ligne (`_short_exc`), et **jamais** un `options_alpaca_{SYM}.csv` d'âge quelconque. Résout m2 au passage. | `test_snapshot_failure_never_serves_cached_chain_as_current_iv` (espion sur `download_options_alpaca` : 0 appel) |
| M2 | `iv_regime` n'est plus dérivé du percentile IV-dans-RV (toujours `None`) ; le chip « Signal (IV) » est remplacé par une légende explicite (prime de risque de vol, pas un IV rank) ; déviation legacy désormais déclarée dans le docstring d'`analytics.py`. | `test_iv_percentile_within_rv_has_no_regime_signal` + `test_tab_shows_no_mean_reversion_signal_on_iv_percentile` (render guard) |
| M3 | Inversion sur le **forward impliqué par parité** (`_forward_from_parity` : F ≈ K₀ + C₀ − P₀ à la paire la plus proche du spot ; repli sur le spot sans paire), puts convertis par parité sur ce même F, et **seul le côté OTM** de chaque strike est inversé (l'ITM est redondant par parité et tombait sous l'intrinsèque non actualisée en basse vol). Mesuré via la vraie fonction : mix déséquilibré **−4 bp** (avant : +62 bp), basse vol σ = 10 % : 7/7 strikes conservés (avant : 10/14 contrats). Limite connue : une chaîne strictement unilatérale (aucune paire call/put) reste inversée sur le spot (+72 / −84 bp). | `test_bs_inversion_is_unbiased_when_call_put_mix_is_unbalanced` (×2), `test_bs_inversion_keeps_one_otm_contract_per_strike_in_low_vol` |
| M4 | `_exchange_today()` = date New York dérivée d'un `_utc_now()` injectable ; utilisée pour les filtres d'échéance, le DTE/T et la clé du cache. | `test_cache_row_is_keyed_by_new_york_date`, `test_dte_uses_new_york_date` (horloge figée à 19 h 30 UTC = 03 h 30 SGT) |
| M5 | `tests/test_iv_dashboard_service.py` : 25 tests (pagination/cap/clés, `_decode_opra`/`_snapshot_mid`/`_snapshot_iv`, greeks + inversion, aller-retour cache, chaîne de fallback des barres, dégradation de l'orchestrateur) + les 7 tests M1–M4 ci-dessus. | le fichier entier |

Non traités dans cette passe (inchangés) : m1, m3–m23 — m2 est résolu par M1 ; m6 ne l'est que sur le chemin snapshots (les deux lignes de `fetch_daily_closes` gardent le texte brut).
...
- Par finding (`p4_<id>_repro.py`) : `stale-chain-cache-as-current-iv`, `iv-signal-vrp-bias`, `parity-r-q-zero-bias` (+`_repro2`), `local-date-vs-exchange-date`, `service-zero-unit-tests`, `snapshot-page-cap-silent`, `fallback-chain-cannot-reach-30dte-and-masks-root-cause`, `ask_only_and_crossed_mids`, `iv-cache-corrupt-file-silent-loss`, `iv-history-filename-unsanitized-symbol`, `html-exception-text-in-log`, `linreg-scipy-valueerror-coincidence`, `regime-split-out-of-range`, `percentile-label-vs-effective-window`, `duplicate-date-crash`, `rv-bad-close-silent-drop`, `iv-disabled-says-alpaca-inaccessible`, `stale-result-under-error-and-params-drift`, `iv-history-overlay-unbounded-x-range`, `chart-title-overprinted-by-legend`, `split-annotation-units-mismatch`, `copy-en-dessous-hyphen`, `forbid-streamlit-script-trips-on-docstring`, `controller-bounds-duplicated-silent-clamp`, `smoke-tab-count-and-inventory`, `render-guard-happy-path-only`, `render-guard-not-in-ci-gate`, `weak-realized-vol-oracle`, `hardcoded-dark-palette-light-theme`, `yahoo-period-string`.
