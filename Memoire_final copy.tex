...
quantitative, sans chercher \`a d\'evelopper ici toutes les d\'erivations pas \`a pas.
Les d\'eveloppements d\'etaill\'es sont regroup\'es plus loin dans le dossier (Heston :
Section~\ref{sec:deriv-heston}, fBM/Davies--Harte : Section~\ref{sec:deriv-fbm},
rough Bergomi/Volterra : Section~\ref{sec:deriv-rbergomi}, SABR : Section~\ref{sec:deriv-sabr},
rough Heston : Section~\ref{sec:deriv-rough-heston}).
...
est d\'evelopp\'ee en Sections~\ref{sec:deriv-fbm} et~\ref{sec:deriv-rbergomi}.}
...
Les d\'erivations Riccati compl\`etes (équations pour $C$ et $D$) sont d\'etaill\'ees en
Section~\ref{sec:deriv-heston}.
...
\begin{description}[style=nextline,leftmargin=2.8cm]
  ...
  Cadre Carr--Madan : damping, transform\'ee et FFT pour prix europ\'eens \`a partir de $\phi_T$ (voir Section~\ref{sec:numerical-methods}).
...
\begin{description}[style=nextline,leftmargin=2.8cm]
  ...
  Conditions d'absence d'arbitrage (butterfly/calendar) pour SVI ; utile si l'on veut une surface param\'etrique robuste (chapitre E).
\end{description}
...
\begin{description}[style=nextline,leftmargin=2.8cm]
  ...
  Mod\`ele affine de r\'ef\'erence ; fonction caract\'eristique et pricing semi-analytique (voir Section~\ref{sec:deriv-heston}).

  \item[\textbf{Hagan, P., Kumar, D., Lesniewski, A., Woodward, D. (2002).}] \emph{Managing Smile Risk}.\\
  Formule asymptotique SABR (Hagan) et interpr\'etation des param\`etres ; standard de calibration (voir Section~\ref{sec:deriv-sabr}).
  ...
  Questions num\'eriques et mod\'elisation associ\'ees \`a la rough volatility (voir Section~\ref{sec:deriv-rough-heston}).

  \item[\textbf{El Euch, O., Rosenbaum, M. (2019).}] \emph{The characteristic function of rough Heston models}.\\
  Rough Heston : cadre Volterra, fonction caract\'eristique et pricing (voir Section~\ref{sec:deriv-rough-heston}).
\end{description}
...
\begin{description}[style=nextline,leftmargin=2.8cm]
  ...
  R\'ef\'erence g\'en\'erale sur MDP, Bellman, MC/TD et Q-learning.

  \item[\textbf{Bertsekas, D. (2012).}] \emph{Dynamic Programming and Optimal Control}.\\
  Point de vue contr\^ole/DP compl\'ementaire, utile pour relier couverture et optimisation.

  \item[\textbf{Mnih, V. et al. (2015).}] \emph{Human-level control through deep reinforcement learning}.\\
  DQN : approximation de $Q$ par r\'eseau et stabilisation (experience replay, targets).

  \item[\textbf{Schulman, J. et al. (2017).}] \emph{Proximal Policy Optimization Algorithms}.\\
  PPO : stabilisation des policy gradients via clipping.
\end{description}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Pistes de compl\'ements (au-del\`a du dossier)}
