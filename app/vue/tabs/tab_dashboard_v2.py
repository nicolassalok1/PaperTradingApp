...
from app.controller import pnl_attribution_controller as pnl_ctrl
...
def _render_charts(lookback_days: int):
    ...
    with left:
        ...
        if eq_curve:
            ...
            st.line_chart(df_eq.set_index("date")[["equity", "portfolio_value"]], use_container_width=True)
        else:
    ...
    with right:
        ...
        if pnl_ts:
            ...
            st.line_chart(df_pnl.set_index("date")[["pnl", "cum_pnl"]], use_container_width=True)
        if vol_ts:
...
def _render_exposure():
    ...
    if data:
        ...
        st.bar_chart(df.set_index("symbol")["market_value"], use_container_width=True)
    ...
    if sectors:
        ...
        st.bar_chart(df_sec.set_index("sector")["market_value"], use_container_width=True)


def _render_pnl_attribution():
    st.subheader("PnL Attribution by Symbol")
    pnl_attr = pnl_ctrl.get_pnl_attribution()
    ...
    if by_symbol:
        ...
        if "unrealized_pnl" in df_attr.columns and "symbol" in df_attr.columns:
            st.bar_chart(df_attr.set_index("symbol")["unrealized_pnl"], use_container_width=True)
    else:
    ...
    c1, c2 = st.columns(2)
    c1.metric("Total Unrealized PnL", f"${total_unreal:,.2f}", delta=f"{total_unreal:,.2f}")
    c2.metric("Total Realized PnL", f"${total_real:,.2f}", delta=f"{total_real:,.2f}")


def _render_trade_history():
    st.subheader("Trade History (Alpaca)")
    days_back = st.number_input("Days back", min_value=1, max_value=365, value=30, step=1)
    limit = st.number_input("Max orders", min_value=10, max_value=500, value=200, step=10)
    ...
    if symbol_filter:
        trades = [t for t in trades if (t.get("symbol") or "").upper() == symbol_filter]
    if trades:
...
def render_tab() -> None:
    ...
    lookback_days = st.slider("Lookback (days)", min_value=30, max_value=365, value=90, step=15)
