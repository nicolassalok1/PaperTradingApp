...
from app.vue.components.options.pricing import price_basket_put


def render():
    option_panel("Basket Put (MC)")
    base = "basket_put_"
    ...
    if compute_button():
        try:
            ...
            price = price_basket_put(weights, spots, r, q, T, sigma, n_paths=int(n_paths))
            st.success(f"Prix Basket Put ≈ {price:.4f}")
        except Exception as exc:
            st.error(f"Echec du pricing Basket Put : {exc}")
