...
from app.vue.components.options.pricing import price_asian_mc
from app.vue.components.options.controller_bridge import *


def render():
    option_panel("Option asiatique (arithmetique)", "Monte Carlo")
    base = "path_asian_arithm_mc_"
    ...
    if compute_button("Pricer (MC)"):
        try:
            ...
            st.success(f"Prix Asian {option_type} ~ {price:.4f}")
        except Exception as exc:
            st.error(f"Echec du pricing Asian : {exc}")
