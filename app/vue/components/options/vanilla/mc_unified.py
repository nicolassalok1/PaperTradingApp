...
def render_mc_panel(default_style: str = "european"):
    ...
    with cols[0]:
        ...
        option_type = st.selectbox("Type", ["call", "put"], index=0, key=_k("mc_type"))
        T = st.slider("Maturité (années)", min_value=0.05, max_value=2.0, value=0.5, step=0.05, key=_k("mc_T"))
    with cols[1]:
        strike = st.slider(
            "Strike",
            min_value=0.5 * S0,
        ...
        sigma = st.slider("Volatilité", min_value=0.05, max_value=1.0, value=0.25, step=0.01, key=_k("mc_sigma"))
        n_paths = st.number_input("N paths", min_value=2000, max_value=50000, value=20000, step=2000, key=_k("mc_npaths"))
    with cols[2]:
        n_steps = st.number_input("N steps (temps)", min_value=25, max_value=1000, value=252, step=25, key=_k("mc_nsteps"))
        if style_choice == "bermudan":
            ex_ctrl = st.number_input("Nb dates d'exercice", min_value=1, max_value=50, value=8, step=1, key=_k("mc_berm_ndates"))
        elif style_choice == "american":
            ex_ctrl = st.number_input("Fréquence exercice (par an)", min_value=50, max_value=500, value=252, step=25, key=_k("mc_am_freq"))
        else:
            ex_ctrl = None
