...
from app.vue.components.options.pricing import price_strangle


def render():
    option_panel("Strangle")
    base = "strangle_"
    ...
    if compute_button("Calculer le prix"):
        try:
            price = price_strangle(S0, K_put, K_call, r, q, T, sigma)
            st.success(f"Prix : {price:.4f}")
        except Exception as exc:
            st.error(f"Echec du pricing Strangle : {exc}")
