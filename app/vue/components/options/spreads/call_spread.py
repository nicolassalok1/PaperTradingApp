...
from app.vue.components.options.pricing import price_call_spread


def render():
    option_panel("Call spread")
    base = "call_spread_"
    ...
    if compute_button("Calculer le prix"):
        try:
            price = price_call_spread(S0, K_long, K_short, r, q, T, sigma)
            st.success(f"Prix : {price:.4f}")
        except Exception as exc:
            st.error(f"Echec du pricing Call spread : {exc}")
