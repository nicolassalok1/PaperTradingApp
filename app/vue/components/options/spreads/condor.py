...
from app.vue.components.options.pricing import price_condor


def render():
    option_panel("Condor (calls)")
    base = "condor_"
    ...
    if compute_button("Calculer le prix"):
        try:
            price = price_condor(S0, K1, K2, K3, K4, r, q, T, sigma)
            st.success(f"Prix : {price:.4f}")
        except Exception as exc:
            st.error(f"Echec du pricing Condor : {exc}")
