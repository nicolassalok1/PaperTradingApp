...
class lookback_call_option:
    ...
    def price_exact(self, z: float = 1):
        ...
        p = sigma**2 / (2 * r)
        v = (
            (1 + p) * z * norm.cdf(delta_p(tau, z, r, sigma))
            + math.exp(-r * tau) * norm.cdf(-delta_m(tau, z, r, sigma))
            - p * math.exp(-r * tau) * z ** (1 - p) * norm.cdf(-delta_m(tau, 1 / z, r, sigma))
            - z
        )
        return v * S0
    ...
    def price_pde(self, n_t: int, n_s: int) -> np.array:
        ...
        for k in range(0, n_t - 1):
            ...
            b[size - 1] = (
                0  # - p3 * u[k + 1][n_x - 1] - p1 * u[k][n_x - 3] + p4 * u[k][n_x - 2] - p3 * u[k][n_x - 1]
            )
