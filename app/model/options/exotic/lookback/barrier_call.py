...
try:
    from app.model.options.exotic.lookback.aux_functions import (
...
class barrier_call_option:
    ...
    def __init__(
        self, T: float, t: float, S0: float, K: float, B: float, r: float, sigma: float
    ) -> None:
    ...
    def price_exact(self):
        ...
        v = (
            S0
            * (norm.cdf(delta_p(tau, S0 / K, r, sigma)) - norm.cdf(delta_p(tau, S0 / B, r, sigma)))
            - math.exp(-r * tau)
            * K
            ...
            * (S0 / B) ** (-2 * r / sigma**2)
            * (
                norm.cdf(delta_p(tau, B**2 / (K * S0), r, sigma))
                - norm.cdf(delta_p(tau, B / S0, r, sigma))
            )
            + math.exp(-r * tau)
            * K
            * (S0 / B) ** (-2 * r / sigma**2 + 1)
            * (
                norm.cdf(delta_m(tau, B**2 / (K * S0), r, sigma))
                - norm.cdf(delta_m(tau, B / S0, r, sigma))
            )
        )
        return v
