...
Responsibilities:
- Define simple stochastic processes (GBM) and option container.
...
def monte_carlo_simulation(
    option: Option, process: StochasticProcess, n: int, m: int, alpha: float = 0.05
) -> float:
    """
    ...
    confidence_interval = [
        floor_n(price - quantile * np.std(payoffs * discount) / np.sqrt(n), 2),
        floor_n(price + quantile * np.std(payoffs * discount) / np.sqrt(n), 2),
    ]
...
def monte_carlo_simulation_LS(
    ...
    print(f"The price of {option!r} = {floor_n(price, 4)}")
