...
def price_european_call_bs(
    ...
    """
    Thin wrapper around Black-Scholes call pricing used by tests.
    Defaults mirror bs_price_call signature while keeping a compact call style.
    """
    return bs_price_call(S, K, r=r, q=q, sigma=sigma, T=T)


def fetch_spy_history(
    period: str = "1y", interval: str = "1d", cache_path: Path | None = None, ticker: str = "SPY"
) -> pd.Series:
...
def payoff_iron_condor(
    spot, k_put_long: float, k_put_short: float, k_call_short: float, k_call_long: float
):
    return (
        payoff_put(spot, k_put_long)
...
def price_strangle_bs(
    ...
    return bs_price_put(S, k_put, r=r, q=q, sigma=sigma_p, T=T) + bs_price_call(
        S, k_call, r=r, q=q, sigma=sigma_c, T=T
    )
...
def pricing_strangle_bs(
    ...
    return price_strangle_bs(
        S, k_put, k_call, r=r, q=q, sigma=sigma, T=T, sigma_call=sigma_call, sigma_put=sigma_put
    )
...
def price_call_spread_bs(
    ...
    return bs_price_call(S, k_long, r=r, q=q, sigma=sigma_l, T=T) - bs_price_call(
        S, k_short, r=r, q=q, sigma=sigma_s, T=T
    )
...
def price_put_spread_bs(
    ...
    sigma_l = sigma if sigma_long is None else sigma_long
    sigma_s = sigma if sigma_short is None else sigma_short
    return bs_price_put(S, k_long, r=r, q=q, sigma=sigma_l, T=T) - bs_price_put(
        S, k_short, r=r, q=q, sigma=sigma_s, T=T
    )
...
def price_butterfly_bs(
    ...
    return (
        ...
        - 2.0 * bs_price_call(S, k2, r=r, q=q, sigma=sig2, T=T)
...
def view_strangle(
    s0: float, k_put: float, k_call: float, span: float = 0.5, n: int = 300, **kwargs
):
...
def view_call_spread(
    s0: float, k_long: float, k_short: float, span: float = 0.5, n: int = 300, **kwargs
):
...
def view_butterfly(
    s0: float, k1: float, k2: float, k3: float, span: float = 0.5, n: int = 300, **kwargs
):
...
def view_condor(
    s0: float, k1: float, k2: float, k3: float, k4: float, span: float = 0.5, n: int = 300, **kwargs
):
...
def view_iron_butterfly(
    ...
    **kwargs,
    ...
    return _build_view(
        ...
        premium,
        s0,
        (k_put_long, k_center, k_call_long),
        (be_low, be_high),
        span,
        n,
    )
...
def _find_breakevens_from_grid(s_grid: np.ndarray, pnl_grid: np.ndarray) -> Tuple[float, ...]:
    ...
    return tuple(sorted(set(floor_n(r, 6) for r in roots)))
...
def view_asset_or_nothing(s0: float, strike: float, T: float = DEFAULT_T, **kwargs):
    ...
    s_grid = np.linspace(s0 * 0.5, s0 * 1.5, 300)
    payoff_grid = payoff_asset_or_nothing(
        s_grid, strike, option_type=kwargs.get("option_type", "call")
    )
    pnl_grid = payoff_grid - premium
    bes = _find_breakevens_from_grid(s_grid, pnl_grid)
    return {
...
def view_quanto(
    s0: float, strike: float, span: float = 0.5, n: int = 300, fx_rate: float = 1.0, **kwargs
):
    premium = price_quanto_bs(s0, strike, fx_rate=fx_rate, **kwargs)
    payoff_fn = lambda s, k: payoff_quanto(
        s, k, option_type=kwargs.get("option_type", "call"), fx_rate=fx_rate
    )
...
def view_rainbow(s0: float, s0b: float, strike: float, span: float = 0.5, n: int = 300, **kwargs):
    ...
    payoff_grid = payoff_rainbow(
        s_grid, s_grid_b, strike, option_type=kwargs.get("option_type", "call")
    )
...
def view_asian_arith(
    s0: float, strike: float, avg_price: float, span: float = 0.5, n: int = 300, **kwargs
):
    ...
    payoff_grid = np.array(
        [
            payoff_asian_arith(
                avg_price * (s / s0), strike, option_type=kwargs.get("option_type", "call")
            )
            for s in s_grid
        ]
    )
...
def view_lookback(
    ...
    **kwargs,
    ...
    payoff_grid = payoff_lookback_floating(
        s_grid, min_path, max_path, option_type=kwargs.get("option_type", "call")
    )
    pnl_grid = payoff_grid - premium
    bes = _find_breakevens_from_grid(s_grid, pnl_grid)
    return {
...
def payoff_lookback_fixed(
    min_path: float, max_path: float, strike: float, option_type: str = "call"
):
    if option_type == "put":
...
def view_forward_start(
    s0: float, spot_start: float, m: float = 1.0, span: float = 0.5, n: int = 300, **kwargs
):
    k_eff = m * spot_start
    opt_type = kwargs.get("option_type", "call")
    premium = (
    ...
    payoff_grid = np.array(
        [
            payoff_forward_start(s, spot_start, m=m, option_type=kwargs.get("option_type", "call"))
            for s in s_grid
        ]
    )
    pnl_grid = payoff_grid - premium
...
def price_calendar_spread_bs(
    ...
    if option_type == "put":
        return bs_price_put(S, K, r=r, q=q, sigma=sigma, T=T_long) - bs_price_put(
            S, K, r=r, q=q, sigma=sigma, T=T_short
        )
    return bs_price_call(S, K, r=r, q=q, sigma=sigma, T=T_long) - bs_price_call(
        S, K, r=r, q=q, sigma=sigma, T=T_short
    )


def view_calendar_spread(
    s0: float,
    strike: float,
    T_short: float,
    T_long: float,
    ...
    **kwargs,
    ...
    payoff_grid = (payoff_call if kwargs.get("option_type", "call") != "put" else payoff_put)(
        np.linspace(s0 * (1.0 - span), s0 * (1.0 + span), n), strike
    )
    pnl_grid = payoff_grid - premium
...
def price_diagonal_spread_bs(
    ...
    if option_type == "put":
        return bs_price_put(S, k_far, r=r, q=q, sigma=sigma, T=T_far) - bs_price_put(
            S, k_near, r=r, q=q, sigma=sigma, T=T_near
        )
    return bs_price_call(S, k_far, r=r, q=q, sigma=sigma, T=T_far) - bs_price_call(
        S, k_near, r=r, q=q, sigma=sigma, T=T_near
    )


def view_diagonal_spread(
    s0: float,
    k_near: float,
    k_far: float,
    T_near: float,
