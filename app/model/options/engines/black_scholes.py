...
def price_basket_call(weights, spots, r, q, T, sigma, n_paths=5000, seed=None):
    weights = np.array(weights, dtype=float)
    spots = np.array(spots, dtype=float)
    if weights.size == 0 or spots.size == 0 or weights.size != spots.size:
        return 0.0
    weights = weights / weights.sum()
    rng = np.random.default_rng(seed)
    dt = float(T)
    z = rng.standard_normal((int(n_paths), len(weights)))
    sims = spots * np.exp((r - q - 0.5 * sigma**2) * dt + sigma * math.sqrt(dt) * z)
    basket = (sims * weights).sum(axis=1)
    payoff = np.maximum(basket - np.average(spots, weights=weights), 0.0)
    return float(math.exp(-float(r) * float(T)) * payoff.mean())


def price_basket_put(weights, spots, r, q, T, sigma, n_paths=5000, seed=None):
    weights = np.array(weights, dtype=float)
    spots = np.array(spots, dtype=float)
    if weights.size == 0 or spots.size == 0 or weights.size != spots.size:
        return 0.0
    ...
    sims = spots * np.exp((r - q - 0.5 * sigma**2) * dt + sigma * math.sqrt(dt) * z)
    basket = (sims * weights).sum(axis=1)
    payoff = np.maximum(np.average(spots, weights=weights) - basket, 0.0)
    return float(math.exp(-float(r) * float(T)) * payoff.mean())
