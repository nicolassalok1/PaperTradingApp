...
def _yfinance_disabled() -> bool:
    """Allow disabling yfinance downloads via env flag."""
    import os
...
def download_yield_curve_to_cache(period: str = "1y") -> Tuple[pd.DataFrame | None, Path | None]:
    """Télécharge les taux Treasury et les écrit dans le cache CSV."""
    ...
    if isinstance(df_raw.columns, pd.MultiIndex):
        ...
        df_close = df_raw
    df_close = df_close.rename(columns={v: k for k, v in YIELD_TICKERS.items()})
    df_close = df_close[[c for c in ("3M", "5Y", "10Y", "30Y") if c in df_close.columns]]
...
def load_yield_curve_csv(ensure_cache: bool = True):
    ...
    for path in candidates:
        ...
        try:
            ...
            pass
        return df, path
    if ensure_cache and not _yfinance_disabled():
