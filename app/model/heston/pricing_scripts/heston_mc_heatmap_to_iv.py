...
def bs_price(
    S0: float, K: float, T: float, vol: float, r: float, option_type: str = "call"
) -> float:
...
def plot_heatmap(
    data: np.ndarray, K_grid: np.ndarray, S_grid: np.ndarray, title: str, zlabel: str = "Value"
):
...
if __name__ == "__main__":
    ...
    # Heatmaps of prices
    fig_call_price = plot_heatmap(
        call_prices, K_grid, S_grid, "Heston Call Prices (Monte Carlo)", "Price"
    )
    fig_put_price = plot_heatmap(
        put_prices, K_grid, S_grid, "Heston Put Prices (Monte Carlo)", "Price"
    )

    # Heatmaps of IVs
    fig_call_iv = plot_heatmap(
        call_iv_surface, K_grid, S_grid, "Black-Scholes IV from Heston Call Prices", "IV"
    )
    fig_put_iv = plot_heatmap(
        put_iv_surface, K_grid, S_grid, "Black-Scholes IV from Heston Put Prices", "IV"
    )
