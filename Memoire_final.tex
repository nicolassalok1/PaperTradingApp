...
pour la fonction caractéristique, inversion de Fourier) est développée en
Section~\ref{sec:deriv-heston}.
...
Les d\'etails (construction du fBM, noyaux, choix de sch\'emas) sont d\'evelopp\'es en
Sections~\ref{sec:deriv-fbm} et~\ref{sec:deriv-rbergomi} ; on renvoie sinon \`a la litt\'erature
sur la rough volatility pour des approfondissements.
...
\begin{itemize}
  \item Observation continue : barrière active en tout temps $\rightarrow$ hitting times exacts.
...
La r\'esolution explicite de ces EDO (voir Section~\ref{sec:deriv-heston}) donne des
...
et des int\'egrales de pricing est fournie en Section~\ref{sec:deriv-heston}.
...
asymptotique en $\nu$ ; une d\'erivation pas \`a pas est donn\'ee en Section~\ref{sec:deriv-sabr}.
...
sont d\'evelopp\'es plus loin (Sections~\ref{sec:deriv-fbm} et~\ref{sec:deriv-rbergomi}) ; on se limite ici aux d\'efinitions,
\`a l'intuition et aux implications pour la g\'eom\'etrie court terme du smile.
...
\begin{itemize}
  ...
  \item Le \textbf{swap de taux} échange flux fixes et variables, permettant de
        transformer exposition fixe $\leftrightarrow$ variable.
\end{itemize}
...
Cette section se lit comme un mini-cours de mod\'elisation de volatilité ``avanc\'ee'' :
on part des objets (noyaux de Volterra, fBM, mod\`eles affines), on relie ces choix
aux faits stylis\'es (smile/skew court terme), puis on d\'eroule pas \`a pas deux
outils omnipr\'esents en pratique :
\begin{itemize}
  \item la simulation du mouvement brownien fractionnaire via la m\'ethode de Davies--Harte (brique centrale en volatilité rugueuse),
  \item la fonction caract\'eristique du mod\`ele de Heston et la valorisation par Fourier (type Carr--Madan).
\end{itemize}
Les compl\'ements (construction rough Bergomi/Volterra, SABR, Rough Heston, m\'ethodes num\'eriques)
sont donn\'es plus loin (Sections~\ref{sec:deriv-rbergomi}, \ref{sec:deriv-sabr}, \ref{sec:deriv-rough-heston}, \ref{sec:numerical-methods}).

\paragraph{Objectifs p\'edagogiques (niveau M1).}
\begin{itemize}
  \item Expliquer le r\^ole du param\`etre de Hurst $H$ et la notion de m\'emoire (rough : $H<1/2$).
  \item Passer d'une covariance th\'eorique \`a un algorithme de simulation (embedding circulant + FFT) et en comprendre les contraintes.
  \item D\'eriver une fonction caract\'eristique dans un mod\`ele affine (ansatz $\rightarrow$ Riccati) et l'utiliser pour pricer une call par Fourier.
  \item Mettre en regard les hypoth\`eses de mod\`ele avec la calibration et les diagnostics num\'eriques.
\end{itemize}
...
\emph{On d\'etaille ci-dessous la simulation du fBM (Davies--Harte), brique centrale des mod\`eles rough ; la construction compl\`ete rough Bergomi/Volterra est donn\'ee en Section~\ref{sec:deriv-rbergomi}.}

\paragraph{Pourquoi Davies--Harte (vue pratique).}
Simuler un fBM revient \`a simuler un vecteur gaussien corr\'el\'e (le \emph{fractional Gaussian noise}).
La m\'ethode de Davies--Harte utilise un embedding circulant et une FFT, ce qui ram\`ene le co\^ut \`a $O(N\log N)$ (au lieu d'une d\'ecomposition de Cholesky dense).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%==========================================================
%  D'erivations int'egr'ees (ex-chapitres 8--9)
%==========================================================
\begingroup
\let\section\subsubsection
\let\subsection\paragraph
\section{Mouvement brownien fractionnaire : m\'ethode de Davies--Harte (d\'erivation pas \`a pas)}
\label{sec:deriv-fbm}
...
On donne maintenant une d\'erivation pas \`a pas : ansatz affine, \'equation de Riccati,
puis pricing par Fourier (Carr--Madan) et remarques num\'eriques.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begingroup
\let\section\subsubsection
\let\subsection\paragraph
\section{D\'erivation pas \`a pas : mod\`ele de Heston}
\label{sec:deriv-heston}
...
%----------------------------------------------------------
% Pr\'erequis pour le DQN : rappels ML / r\'eseaux
\subsection{Pr\'erequis : apprentissage supervis\'e}
...
\]
Ce sont les modèles sous-jacents à DALL$\cdot$E 2, Stable Diffusion, Imagen.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Deep Q-Network (DQN) : comprendre le m\'ecanisme pas \`a pas}

Le DQN (Deep Q-Network) est l'id\'ee de Q-learning appliqu\'ee \`a des espaces d'\'etats continus
ou de grande dimension, en rempla\c{c}ant la table $Q(s,a)$ par un r\'eseau de neurones
$Q_\theta(s,a)$.

\paragraph{Point de d\'epart : l'\'equation de Bellman optimale.}
La fonction $Q^*$ satisfait
\[
Q^*(s,a)=\mathcal{R}(s,a)+\gamma_{\mathrm{RL}}\,\mathbb{E}\!\left[\max_{a'}Q^*(s',a')\mid s,a\right].
\]
Le Q-learning tabulaire est une descente stochastique vers cette identit\'e en utilisant une transition observ\'ee
$(s_t,a_t,r_t,s_{t+1})$.

\paragraph{1. Approximation param\'etrique de $Q$.}
On choisit une famille $Q_\theta(s,a)$ (par exemple un MLP) et l'on veut ajuster $\theta$ pour que
les valeurs pr\'edites satisfassent, en moyenne, l'\'equation de Bellman.

\paragraph{2. Cible TD (temporal-difference target).}
Sur une transition $(s,a,r,s')$, la cible ``Bellman'' naturelle est
\[
y := r + \gamma_{\mathrm{RL}} \max_{a'} Q_\theta(s',a').
\]
On cherche \`a rendre $Q_\theta(s,a)$ proche de $y$.

\paragraph{3. Perte : l'erreur de Bellman (MSE).}
Le DQN minimise une perte de type
\[
L(\theta)=\mathbb{E}\big[(y - Q_\theta(s,a))^2\big],
\]
o\`u l'esp\'erance est approxim\'ee par une moyenne sur un mini-batch de transitions.
La mise \`a jour se fait par descente de gradient :
\[
\theta \leftarrow \theta - \eta\,\nabla_\theta L(\theta).
\]

\paragraph{Pourquoi le sch\'ema na\"if diverge souvent.}
Si l'on utilise le m\^eme r\'eseau pour pr\'edire $Q_\theta(s,a)$ et construire la cible $y$,
alors la cible bouge \`a chaque it\'eration (\emph{moving target}) et l'on apprend sur des donn\'ees tr\`es corr\'el\'ees
dans le temps (trajectoire), ce qui cr\'ee des instabilit\'es.
Le DQN stabilise l'apprentissage par deux id\'ees cl\'es.

\paragraph{4. Experience replay : d\'ecorr\'eler et r\'eutiliser les donn\'ees.}
On stocke des transitions $(s,a,r,s',\mathrm{done})$ dans un buffer $\mathcal{D}$, puis on entra\^{i}ne sur des mini-batches
\emph{tir\'es al\'eatoirement} dans $\mathcal{D}$. Cela :
\begin{itemize}
  \item casse les corr\'elations temporelles (les mini-batches ressemblent \`a des tirages i.i.d.) ;
  \item am\'eliore l'efficacit\'e \'echantillon (une transition est r\'eutilis\'ee plusieurs fois) ;
  \item lisse l'\'evolution des cibles (les donn\'ees vues par le r\'eseau varient moins brutalement).
\end{itemize}

\paragraph{5. Target network : figer la cible pendant quelques pas.}
On introduit un second r\'eseau $Q_{\theta^-}$ (copie retard\'ee) utilis\'e uniquement pour construire la cible :
\[
y := r + \gamma_{\mathrm{RL}} \max_{a'} Q_{\theta^-}(s',a').
\]
Les param\`etres $\theta^-$ sont mis \`a jour p\'eriodiquement (toutes les $C$ it\'erations) par
copie : $\theta^- \leftarrow \theta$. Ainsi, $y$ est (temporairement) une cible plus stable.

\paragraph{6. Politique d'exploration : $\epsilon$-greedy.}
L'agent collecte des donn\'ees avec une politique d'exploration typique :
\[
a =
\begin{cases}
\text{action al\'eatoire} & \text{avec proba. }\epsilon,\\
\arg\max_{a'} Q_\theta(s,a') & \text{avec proba. }1-\epsilon.
\end{cases}
\]
On fait d\'ecro\^{i}tre $\epsilon$ au cours du temps (explorer puis exploiter) afin de construire un replay buffer riche.

\paragraph{7. DQN en une boucle d'apprentissage (vue conceptuelle).}
Sans pseudo-code, l'encha\^{i}nement est :
\begin{enumerate}[leftmargin=*]
  \item initialiser $Q_\theta$ et $Q_{\theta^-}$, et un buffer $\mathcal{D}$ ;
  \item r\'ep\'eter : ex\'ecuter une action (avec exploration), observer $(s,a,r,s',\mathrm{done})$, stocker dans $\mathcal{D}$ ;
  \item tirer un mini-batch dans $\mathcal{D}$ et minimiser $L(\theta)$ avec la cible construite par $Q_{\theta^-}$ ;
  \item toutes les $C$ it\'erations, mettre \`a jour $\theta^-\leftarrow\theta$.
\end{enumerate}

\paragraph{Exemple minimal : calcul d'une cible et d'une perte.}
Consid\'erons une transition $\bigl(s,a,r,s',\mathrm{done}\bigr)$ tir\'ee du replay buffer.
Le DQN construit une cible scalaire
\[
y =
\begin{cases}
r & \text{si }\mathrm{done}=\text{vrai},\\
r + \gamma_{\mathrm{RL}}\max_{a'}Q_{\theta^-}(s',a') & \text{sinon.}
\end{cases}
\]
La contribution de cette transition \`a la perte est alors
\[
\ell(\theta)=\bigl(y - Q_\theta(s,a)\bigr)^2,
\]
et l'actualisation de $\theta$ d\'eplace $Q_\theta(s,a)$ dans la direction de $y$.
Sur un mini-batch, on moyenne simplement ces contributions, ce qui revient \`a ajuster simultan\'ement
les valeurs sur plusieurs couples $(s,a)$ et \`a stabiliser le gradient.

\paragraph{8. Sur-estimation et variantes : Double DQN.}
Le terme $\max_{a'}Q(\cdot)$ introduit un biais de sur-estimation (le maximum d'estimateurs bruit\'es est biais\'e).
Double DQN d\'ecorr\`ele \emph{s\'election} et \emph{\'evaluation} :
\[
y := r + \gamma_{\mathrm{RL}}\, Q_{\theta^-}\!\Bigl(s', \arg\max_{a'} Q_\theta(s',a')\Bigr).
\]

\paragraph{Cas pratique : actions discr\`etes vs continues.}
Le DQN suppose un ensemble d'actions discret. En finance, une couverture naturelle peut \^etre continue (quantit\'e de sous-jacent).
Une strat\'egie standard consiste \`a \emph{discr\'etiser} l'action (par ex.\ des incr\'ements de hedge $\{\ldots,-2\Delta,-\Delta,0,\Delta,2\Delta,\ldots\}$),
ce qui rend le probl\`eme traitable par DQN au prix d'un compromis pr\'ecision/co\^ut de transaction.
Pour des actions continues, on se tourne plut\^ot vers des approches actor--critic (DDPG/TD3/SAC) ; le DQN reste un point d'entr\'ee p\'edagogique robuste.

\paragraph{9. Dueling networks et prioritized replay (options).}
\begin{itemize}
  \item \textbf{Dueling} : d\'ecomposer $Q_\theta(s,a)=V_\theta(s)+A_\theta(s,a)$ pour mieux g\'erer les nombreux $a$.
  \item \textbf{Prioritized replay} : sur-\'echantillonner les transitions \`a forte erreur TD $|y-Q_\theta(s,a)|$.
\end{itemize}

\paragraph{Lecture ``finance'' : DQN comme apprentissage de couverture.}
Dans une formulation MDP de couverture, $s_t$ agr\`ege l'information (temps restant, spot, vol implicite, greeks, etc.),
l'action $a_t$ repr\'esente un ajustement discret de hedge (quantit\'e d'actif sous-jacent, ou position en options de couverture),
et la r\'ecompense p\'enalise le risque et les co\^uts (variance de PnL, drawdown, co\^ut de transaction).
Le DQN apprend alors une approximation de $Q^*(s,a)$ permettant de d\'ecider un hedge discret optimal sous co\^uts et contraintes.

\paragraph{Points de vigilance.}
Le DQN est sensible \`a : (i) l'\'echelle des r\'ecompenses, (ii) la non-stationnarit\'e des donn\'ees (march\'e), (iii) la d\'efinition de l'\'etat
et des actions, et (iv) les contraintes de risque (VaR/ES, limites) \`a int\'egrer dans la r\'ecompense ou via des contraintes.
...
Des sections de d\'erivations pas \`a pas (Heston, fBM, Volterra/rough Bergomi, SABR, rough Heston,
m\'ethodes num\'eriques) compl\`etent le dossier pour servir de base \`a des travaux plus avanc\'es.


%==========================================================
%                 PARTIE 5 : BIBLIOGRAPHIE
%==========================================================
\section{SDE de Volterra et mod\`ele rough Bergomi}
\label{sec:deriv-rbergomi}
...
\section{Mod\`ele SABR : expansion asymptotique (Hagan)}
\label{sec:deriv-sabr}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Formule asymptotique de la vol implicite}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Utilisation en calibration}
...
\begin{itemize}
  ...
  \item La dérivation complète nécessite des développements asymptotiques
        détaillés (au-delà du niveau de détail visé ici ; voir la littérature spécialisée).
\end{itemize}
...
\section{Mod\`ele de Rough Heston : dynamique, caract\'eristique, simulation}
\label{sec:deriv-rough-heston}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Dynamique du Rough Heston}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Formulation Volterra}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Fonction caract\'eristique du log--prix}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Prix du call europ\'een}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Simulation : schéma hybride (hybrid scheme)}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{M\'ethodes num\'eriques : FFT et Crank--Nicolson}
\label{sec:numerical-methods}

\subsection*{M\'ethode de Crank--Nicolson pour la PDE de Black--Scholes}
...
\subsection*{FFT pour le pricing d'options (Carr--Madan)}
...
\section{\'Etude de cas : pipeline de pr\'ediction tabulaire par eras (Numerai)}
\label{sec:case-numerai}

\subsection*{Objectif et positionnement}
Cette section pr\'esente, au niveau conceptuel (sans code), un \emph{pipeline} de pr\'ediction tabulaire structur\'e par p\'eriodes (``eras'') tel
...
Cette \'etude de cas illustre les principes d\'evelopp\'es au chapitre sur l'apprentissage supervis\'e sous non-stationnarit\'e
